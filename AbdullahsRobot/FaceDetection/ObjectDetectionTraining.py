...
def image_bb_transforms(image : Type[Image], bboxes : List[BoundingBox]) -> Type[torch.FloatTensor], List[BoundingBox]:
...
class ObjectDetectionTrainer:
    ...
    """
    def __init__(self, max_epoch : int, dataset_path : str, output_grid_size : int, anchor_boxes : List[BoundingBox], target_device : str, target_image_size : Tuple[int,int] = (320,320), n_classes : int = 1):
        ...
        self.dataset = ObjectDetectionDataset(dataset_path, target_image_size, transform=image_bb_transforms)
    ...
    def parse_the_outputs(self, output : Type[torch.FloatTensor], boxes : List[List[BoundingBox]], classes : List[List[int]]):
        ...
        for image_id, bb_list in enumerate(boxes):
            ...
            for bb_id, bb in enumerate(bb_list):
                ...
                if matching_anchors:
                    ...
                    positive_classprob_output.append(class_slices[bbox_index])
                    positive_classprob_target.append(class_target)
        ...
        positive_classprob_output = class_slices[positive_bbox_indices_list]
        ...
        return positive_bbox_outputs, negative_bbox_outputs, bbox_targets, positive_classprob_output, anchor_sizes
    ...
    def train_batch(self, batch_data : Type[torch.Tensor], epoch : int):
        """Train batch
        ...
        positive_bbox, negative_bbox, positive_bbox_targets, positive_classprob_output, anchor_sizes = self.parse_the_outputs(cell_outputs, boxes)
        ...
        # calc bb loss
        bb_loss_offset = nn.functional.mse_loss(positive_bbox_output[:,:2], positive_bbox_targets[:,:2], reduction='sum')
    ...
    def start_training(self):
        dataloader = DataLoader(self.dataset, 16, True, collate_fn = object_dataset_collate_fn, num_workers=6, worker_init_fn=worker_init_fn)
